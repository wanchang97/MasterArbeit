...
close all;
clc
addpath('C:\MasterArbeit\WanchangMasterArbeit4\CEsBU')
...
a0_initial = 0.15; % mm
ac = 50;% mm
T_final = 50;T_object_list = [5,10,15,20,25,30,35,40,45];
T_final_text = ['T_{final}= ',num2str(T_final,'%g ')];
% RVs_prior distribution
% a0~exp(mu=1,si=mu=1); ds~normal(mu=60,si=10),lnC, m~ multinormal(mu = [-33;3.5] si = [0.47,0.3],correlation coefficient = -0.9)
% Units are N and mm
lambda_a0 = 1; mu_a0 = 1/lambda_a0; si_a0 = 1/lambda_a0; % lambda_a0 > 0
mu_DS = 60; si_DS = 10;
mu_lnC = -33; si_lnC = 0.47;
mu_m = 3.5; si_m = 0.3;
prior_name = {'a0','\DeltaS','lnC','m'};
prior_distribution = {'exponential','normal','normal','normal'};
mu_prior = [mu_a0;mu_DS;mu_lnC;mu_m];
si_prior = [si_a0;si_DS;si_lnC;si_m];
rho_lnC_m = -0.9;
% x_given = [mu_a0,mu_DS,mu_lnC,mu_m];
a0 = ERADist('exponential','MOM',[mu_a0]); % 0,01 to 50
DS = ERADist('normal','MOM',[mu_DS,si_DS]);% 10 to 110
...
% Covariance matrix
COV = D*R_XX*D;
dist_prior = ERANataf([a0,DS,lnC,m],R_XX);
%% artificial data
t_m = 1:T_final;M = size(t_m,2);
x_given = [mu_a0+si_a0;mu_DS+si_DS;mu_lnC-si_lnC;mu_m-si_m];
...
annotation('textbox',[0.15,0.6,0.5,0.2],'String','a = a(a0,DS,lnC,m,t)','FontSize',asize,'EdgeColor','none');
xlabel('t_m (year)');
...
mu_prior_text = {};mu_CEsBU1_text = {};x_given_text = {};
...
for d = 1:dim
    ...
    x_given_text{end+1} = ['x_{given}= ', num2str(x_given(d),'%.3f')];
...
for d = 1: dim
    ...
    s = {mu_prior_text{d};x_given_text{d};mu_CEsBU1_text{d}};
    text(0.63,0.8,s,'FontSize',12,'Units','normalized');
    xlabel(['RV ',prior_name{d}]);
    ylabel('PDF')
    
end
sgtitle(['Intermediate dist CEsBU1 in X space for all RVs ',T_final_text]);
...
for i = 1:length(T_object_list)
    ...
    T_object_text = [' T_{object}= ',num2str(T_object,'%g ')];
    mu_a_CEsBU1_object_text = ['mu_{a_{CEsBU1_{object}}}: ',num2str(mu_a_CEsBU1(T_object),'%.3f')];
    si_a_CEsBU1_object_text = ['si_{a_{CEsBU1_{object}}}: ',num2str(si_a_CEsBU1(T_object),'%.3f')];
    ...
    mu_a_CEsBU2 = mean(a_CEsBU2,2); % T_final x1
    si_a_CEsBU2 = std(a_CEsBU2,0,2);
    ...
    for d = 1: dim
        ...
        s = {mu_prior_text{d};x_given_text{d};mu_CEsBU2_text{d}};
        text(0.63,0.8,s,'FontSize',12,'Units','normalized');
        xlabel(['RV ',prior_name{d}]);
        ylabel('PDF')
    end
    sgtitle(['Intermediate dist CEsBU2 in X space for all RVs ',T_object_text]);
    legend(ax{1},[p1,p2,p3,p4,p5],'Location','northwestoutside');
    t.TileSpacing = 'compact';
    t.Padding = 'compact';
    ...
    p5 = xline(T_object,'--k','DisplayName','T_{object}');
    annotation('textbox',[0.15,0.7,0.5,0.2],'String',{Z_SS1_text},'FontSize',16,'EdgeColor','none');
    legend([p1,p2,p3,p4,p5]);
    xlabel('t_m (year)');ylabel('a CrackLength (mm)');
    ...
    %% plot the dist.for Prior; CEsBU1 ;SS1,2,3,4
    figure;
    % figure size setting
    set(gcf,'position',figsize(2,:))
    t = tiledlayout(2,2);
    ...
    for d = 1: dim
        ...
        s = {mu_prior_text{d},x_given_text{d},mu_CEsBU1_text{d},mu_SS1_text{d},mu_SS2_text{d},mu_SS3_text{d},mu_SS4_text{d}};
        text(0.66,0.66,s,'FontSize',10,'Units','normalized');
    end
    ...
    saveas(gcf,name,'png')
    %% plot the dist.for Prior; CEsBU2 ;SS1,2,3
    figure;
    % figure size setting
    set(gcf,'position',figsize(2,:))
    t = tiledlayout(2,2);
    ...
    for d = 1:dim
        ...
        p12 = plot(mu_SS4(d),0,'go','DisplayName','mu_{T_{object}SS4}');
        xlabel(['RV ',prior_name{d}]);  ylabel('PDF');
        s = {mu_prior_text{d},x_given_text{d},mu_CEsBU2_text{d},mu_SS1_text{d},mu_SS2_text{d},mu_SS3_text{d},mu_SS4_text{d}};
        text(0.66,0.66,s,'FontSize',10,'Units','normalized');
    end
    ...
    %% plot the dist.for Prior; CEsBU1,2 ;SS1,2,3
    figure;
    % figure size setting
    set(gcf,'position',figsize(2,:))
    t = tiledlayout(2,2);
    ...
    for d = 1:dim
        ...
        xlabel(['RV ',prior_name{d}]);  ylabel('PDF')
        s = {mu_prior_text{d},x_given_text{d},mu_CEsBU1_text{d},mu_CEsBU2_text{d},mu_SS1_text{d},mu_SS2_text{d},mu_SS3_text{d},mu_SS4_text{d}};
        text(0.66,0.6,s,'FontSize',10,'Units','normalized');
    end
