% plot sample based filled contour of joint PDF
function handle = scattercontourf(samples,bins,levels,color,xlimit,ylimit,varargin)
    if nargin == 1
    ...
    if nargin <= 2
        ...
        handle = contourf(c{1},c{2},n',varargin{:});
    elseif nargin==3
        [n,c] = hist3([samples'],'Nbins',[xbins,ybins]);
        handle = contourf(c{1},c{2},n',levels,varargin{:});
    elseif nargin==4
        [n,c] = hist3([samples'],'Nbins',[xbins,ybins]);
        if isempty(color)
            handle = contourf(c{1},c{2},n',levels,varargin{:});
        else
            handle = contourf(c{1},c{2},n',levels,'LineColor',color,varargin{:});
        end
    elseif nargin==5
        edges = {linspace(xlimit(1),xlimit(2),xbins+1);linspace(xlimit(1),xlimit(2),ybins+1)};
        [n,c] = hist3([samples'],'Edges',edges);
        if isempty(color)
            handle = contourf(c{1},c{2},n',levels);
        else
            handle = contourf(c{1},c{2},n',levels,'LineColor',color,varargin{:});
        end
    elseif nargin==6
