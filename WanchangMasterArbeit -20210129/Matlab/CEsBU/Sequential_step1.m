function [U_object,X_object,Z_object] = Sequential_step1(priorobj,T_object,lv,u_tot,nu_tot,lnLeval_allyear,dim, k, ISdensity,N,nESS)
%{
...
-----------------------------------------------------------------------------------------------
    %}
    fprintf('\nstart Sequential_step1 (with log-scaled input)\n')
    ...
    ln_h_mixture = logsumexp(ln_h_tot)-log(lv);
%     ln_z = zeros(lv,1);
%     ln_w_final_tot = zeros(N,lv);
...
%         ln_w_normalised_cell{end+1} = ln_w_normalised;
        %         ln_w_final = ln_w_normalised + log(pi(t));
%         ln_w_final_tot(:,t) = ln_w_final;
