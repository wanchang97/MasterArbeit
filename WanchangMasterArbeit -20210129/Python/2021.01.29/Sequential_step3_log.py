...
"""
Sequential Bayesian3 updating in log scale
Comments 
...
w_newt : phi(ut)*L(ut)/h_t, where hmixture = sum(1/lv*ht)
----------------------------------------------------------------
Input
...
"""
def Sequential_step3_log(prior,T_object ,lv ,samplesU ,mu_U_list,si_U_list,nESS_list,lnLeval_allyear_list):
    ...
    for t in range(lv):
        ...
        Z = Z + pi_list[t]*np.exp(ln_z)
        ln_w_normalised = ln_w_new - sp.special.logsumexp(ln_w_new)
        w_resample = pi_list[t]*np.exp(ln_w_normalised)
