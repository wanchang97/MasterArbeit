...
######!! This method takes COLUMN VECTOR of random variable !!######
#------------Discard dimension check to speed up------------
...
#------------------------------------------------------------
        Z = self.A @ U
        m = U.shape[0]  # m = No. of dimensions
        # X = np.zeros(U.shape)
        X = list()
        for i in range(m):
            # X[i, :] = self.Marginals[i].icdf(stats.norm.cdf(Z[i, :]))
            X.append(self.Marginals[i].icdf(stats.norm.cdf(Z[i, :])))
        X = np.asarray(X)
        if (not Jacobian):
            return X
        else:
            ...
            return X, Jac
    # def U2X(self,U,Jacobian=False):
    #     n_dim = len(self.Marginals) # number of dimensions of the joint distribution
    #     # check the dimension of input U
    #     if si
    ...
    # %% ----------------------------------------------------------------------------
    def jointpdf(self, x):
######!! This method takes ROW VECTOR of random variable !!######
#------------Discard dimension check to speed up------------
        # x = np.array(x, ndmin=2)
        # if(np.shape(x)[1] == 1):
        #     x = x.T
